...
namespace AdaptiveMusicSample
{
    ...
    /// </remarks>
    public sealed class AdaptiveMidiFileSequencer : IAudioRenderer
    {
        ...
        public AdaptiveMidiFileSequencer(Synthesizer synthesizer)
        {
            if (synthesizer == null)
            {
            ...
            isTrackEnabled = Enumerable.Repeat(true, synthesizer.ChannelCount).ToArray();
        }
        ...
        private void ProcessEvents()
        {
            ...
            while (msgIndex < midiFile.Messages.Length)
            {
                ...
                if (time <= currentTime)
                {
                    if (msg.Type == AdaptiveMidiFile.MessageType.Normal)
                    {
                        if (isTrackEnabled[msg.Channel] || msg.Command != 0x90)
                        {
                    ...
                    {
                        if (msg.Type == AdaptiveMidiFile.MessageType.LoopStart)
                        {
                            loopIndex = msgIndex;
                        }
                        else if (msg.Type == AdaptiveMidiFile.MessageType.LoopEnd)
                        {
        ...
        {
            isTrackEnabled[track] = true;
        }

        public void DisableTrack(int track)
        {
            synthesizer.NoteOffAll(track, false);
            isTrackEnabled[track] = false;
        }
